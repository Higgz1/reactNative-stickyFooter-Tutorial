...
const getImage = (i: any) => `https://source.unsplash.com/600x${400 + i}`;

const Actual = () => {
  const scrollY = React.useRef(new Animated.Value(0)).current;
  const [bottomActions, setBottomActions] = useRecoilState(layoutState);
  const topEdge =
    (height + bottomActions?.height - bottomActions?.y) * 2.6 - 9.5;
  const topEdge2 = 2757 - height + bottomActions?.height;

  console.log({ topEdge2 });

  const inputRange = [-1, 0, topEdge2 - 60, topEdge2, topEdge2 + 1];
  useEffect(() => {}, [bottomActions]);
    ...
    <>
      ...
      >
        ...
        <View>
          ...
          {articleParagraphs.splice(0, 3).map((text, index) => {
            return (
              <View
                key={`featured-${index}`}
